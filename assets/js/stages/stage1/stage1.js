...
function init(){
    return new Promise( ( resolve , reject ) => {
        ...
        if(stage !== 1) reject( new Error('invalid value for stage') );

        let templateInterface = new TemplateInterface( stage , templateAction  ) ;

        if(templateAction === 'create') templateInterface .createTemplate('H')
            .then( templateInterface => resolve(templateInterface) )
            .catch( reject );

        else if (templateAction === 'load') return templateInterface .loadTemplate()
            .then( templateInterface => resolve(templateInterface) )
            .catch( reject );

        else return reject('Invalid action')

    }).then( (templateInterface) => {
